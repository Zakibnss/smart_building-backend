...
try {
    ...
    switch ($method) {
        case 'GET':
            if (isset($_GET['id'])) {
                $stmt = $pdo->prepare("
                    ...
                    FROM utilisateur 
                    WHERE role = 'agent_service' AND id = ?
                ");
        ...
        case 'POST':
            ...
            echo json_encode([
                'success' => true,
                'message' => 'Agent de service ajouté avec succès',
                'id' => $agentId
            ]);
            break;
        ...
        case 'PUT':
            ...
            $sql .= " WHERE id = ? AND role = 'agent_service'";
            $params[] = $_GET['id'];
        ...
        case 'DELETE':
            ...
            try {
                ...
                // Mettre à jour les services (enlever l'agent)
                ...
                $stmt = $pdo->prepare("DELETE FROM utilisateur WHERE id = ? AND role = 'agent_service'");
                $stmt->execute([$userId]);
